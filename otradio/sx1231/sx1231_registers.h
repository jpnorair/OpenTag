...
#define REG_BITSYNC_GAINPROP        0x62
#define REG_BITSYNC_GAININTEG       0x63
#define REG_BITSYNC_TEST            0x64
#define REG_ADCBW                   0x65
#define REG_ADCTEST                 0x66
#define REG_DAGCGAIN                0x67    // aka "DAGCTST"
#define REG_TESTDAGC                0x6f    // aka "IO_MODE"
...
#define RF_GPIO_MODE_NORMAL     0x00
#define RF_GPIO_MODE_DEBUG      0x40
#define RF_GPIO_MODE_FPGA       0x80
#define RF_DAGC_CONTINUOUS      0x20    // true = DAGC updated every two bits
#define RF_DEMOD_SYMMETRY       0x10
...
#define RADIO_STATE_RX          (2 << RADIO_STATE_RXSHIFT)
...
#define RADIO_STATE_DUMMY_TX    (15 << RADIO_STATE_TXSHIFT)
#endif /* DISABLE_TRANSMIT */

/********************************************************************************/

// struct member access:
#define CHIP_MODE_SLEEP              0
#define CHIP_MODE_STANDBY            1
#define CHIP_MODE_SYNTHESIZER        2
#define CHIP_MODE_TRANSMITTER        3
#define CHIP_MODE_RECEIVER           4
...
#define RADIO_FLAG_FRCONT       (1 << 0)
    ...
    #define RADIO_FLAG_FLOOD_STOP        (1 << 2)
#endif
#define RADIO_FLAG_AUTO         (1 << 3)
#define RADIO_FLAG_RESIZE       (1 << 4)

// RSSI measurement during CCA and during packet reception
// CCA measurement will likely be more noisey and require more samples
#define RSSI_SUM_SHIFT        3
#define RSSI_SUM_COUNT        (1 << RSSI_SUM_SHIFT)
...
typedef union {
    struct {    // sx1231 register 0x01
        ot_u8 unused        : 2;    // 0,1
        ot_u8 Mode          : 3;    // 2,3,4
        ot_u8 ListenAbort   : 1;    // 5
        ot_u8 ListenOn      : 1;    // 6
        ot_u8 SequencerOff  : 1;    // 7
    } bits;
    ot_u8 octet;
} RegOpMode_t;

typedef union {
    struct {    // sx1231 register 0x02
        ot_u8 ModulationShaping : 2;    // 0,1
        ot_u8 unused2           : 1;    // 2
        ot_u8 ModulationType    : 2;    // 3,4
        ot_u8 DataMode          : 2;    // 5,6
        ot_u8 unused7           : 1;    // 7
    } bits;
    ot_u8 octet;
} RegDataModul_t;
...
typedef union {
    struct {    // sx1231 register 0x11
        ot_u8 OutputPower   : 5;    // 0,1,2,3,4
        ot_u8 Pa2On         : 1;    // 5
        ot_u8 Pa1On         : 1;    // 6
        ot_u8 Pa0On         : 1;    // 7
    } bits;
...
typedef union {
    struct {    // sx1231 register 0x18
        ot_u8 lna_gain      : 3;    // 0,1,2    // 0=auto (but useless since only done on rx start)
        ot_u8 lna_gain_out  : 3;    // 3,4,5    // read-only
        ot_u8 lna_lowpower  : 1;    // 6
        ot_u8 lna_zin_200   : 1;    // 7
    } bits;
    ot_u8 octet;
} RegLna_t;

typedef union {
    struct {    // sx1231 register 0x19
        ot_u8 RxBw      : 5;    // 0,1,2,3,4
        ot_u8 DccFreq   : 3;    // 5,6,7
    } bits;
    ot_u8 octet;
} RegRxBw_t;

typedef union {
    struct {    // sx1231 register 0x27
        ot_u8 SyncAddressMatch  : 1;    // 0
        ot_u8 AutoMode          : 1;    // 1
        ot_u8 Timeout           : 1;    // 2
        ot_u8 Rssi              : 1;    // 3
        ot_u8 PllLock           : 1;    // 4
...
typedef union {
    struct {    // sx1231 register 0x37
        ot_u8 crc_ibm           : 1;    // 0        CC1100 crc  (with whitening only?)
        ot_u8 AddressFiltering  : 1;    // 1,2
        ot_u8 CrcAutoClearOff   : 1;    // 3
        ot_u8 CrcOn             : 1;    // 4
        ot_u8 DcFree            : 2;    // 5,6
        ot_u8 PacketFormat      : 1;    // 7
    } bits;
    ot_u8 octet;
} RegPacketConfig1_t;

typedef union {
    struct {    // sx1231 register 0x3c
        ot_u8 FifoThreshold     : 7;    // 0,1,2,3,4,5,6
        ot_u8 TxStartCondition  : 1;    // 7
    } bits;
    ot_u8 octet;
} RegFifoThresh_t;
...
typedef union {
    struct {    // sx1231 register 0x64
        ot_u8 bsync_test_mode           : 1;    // 0,1,2
        ot_u8 bsync_adapt_notransition  : 1;    // 3
        ot_u8 bsync_thresh_validity     : 1;    // 4
        ot_u8 unused                    : 3;    // 5,6,7
    } bits;
    ot_u8 octet;
} RegBitSyncTest_t;

typedef union {
    struct {    // sx1231 register 0x65
        ot_u8 adc_bw        : 3;    // 0,1,2        manual ADC bandwidth value
        ot_u8 adc_bw_auto   : 1;    // 3            defaults to set, auto bw from rxbw
        ot_u8 unused        : 4;    // 4,5,6,7
    } bits;
    ot_u8 octet;
} RegAdcBw_t;

typedef union {
    struct {    // sx1231 register 0x67
        ot_u8 dagc_gain         : 4;    // 0,1,2,3
        ot_u8 dagc_manual       : 1;    // 4
        ot_u8 unused            : 2;    // 5,6
        ot_u8 rx_cal_timeout    : 1;    // 7        read-only
    } bits;
    ot_u8 octet;
} RegDagcTest_t;

typedef union {
    struct {    // sx1231 register 0x6d
        ot_u8 demod_ook_bias    : 1;    // 0,1
        ot_u8 demod_peak_recov  : 1;    // 2
        ot_u8 demod_lim         : 1;    // 3    ?
        ot_u8 crc_seed_en       : 1;    // 4
        ot_u8 unused            : 2;    // 5,6
        ot_u8 on_rx             : 1;    // 7
    } bits;
    ot_u8 octet;
