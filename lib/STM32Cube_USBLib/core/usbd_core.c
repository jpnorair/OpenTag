...
USBD_StatusTypeDef USBD_DeInit(USBD_HandleTypeDef *pdev) {
    pdev->dev_state = USBD_STATE_DEFAULT;           /* Set Default State */
    pdev->pClass->DeInit(pdev, pdev->dev_config);   /* Free Class Resources */
    USBD_LL_Stop(pdev);                             /* Stop the low level driver  */
    USBD_LL_DeInit(pdev);                           /* Initialize low level driver */
    return USBD_OK;
}
...
USBD_StatusTypeDef USBD_Start(USBD_HandleTypeDef *pdev) {
    USBD_LL_Start(pdev);    /* Start the low level driver  */
    return USBD_OK;  
}
