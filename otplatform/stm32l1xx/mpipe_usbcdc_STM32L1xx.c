...
///        which will get stuck during debugging or other sleeping processes.   </LI>
    ...
    cdcacm.seq.ushort       = 0;                        // not actually necessary
...
void sub_txopen() {
    ...
    cdcacm.packets -= 1;
    cdcacm.pktlen   = cdcacm.pkt[2] + MPIPE_OVERHEADBYTES;
    sub_usb_loadtx();
}
...
void mpipedrv_rxndef(ot_bool blocking, mpipe_priority data_priority) {
    ...
    if (mpipe.state == MPIPE_Idle) {
        mpipe.state     = MPIPE_Idle;
        cdcacm.pktlen   = MPIPE_OVERHEADBYTES;
...
void mpipedrv_isr() {
    switch (mpipe.state) {
        ...
        case MPIPE_RxPayload: 
            if (cdcacm.pkt >= mpipe.alp.inq->back) {
                /// The reception is completely done.  Go to Idle and do callback.
                mpipe.alp.inq->back    -= MPIPE_FOOTERBYTES;
                mpipe.state             = MPIPE_Idle;
                mpipeevt_rxdone(0);
            }
            break;
