...
/** Platform Debug Triggers <BR>
  ...
  */
#ifdef OT_TRIG1_PORT
void trigger(TR1, high) {    OT_TRIG1_PORT->ODR |= OT_TRIG1_PIN; }
void trigger(TR1, low) {     OT_TRIG1_PORT->ODR &= ~OT_TRIG1_PIN; }
void trigger(TR1, toggle) {  OT_TRIG1_PORT->ODR ^= OT_TRIG1_PIN; }
#else
...
#ifdef OT_TRIG2_PORT
void trigger(TR2, high) {    OT_TRIG2_PORT->ODR |= OT_TRIG2_PIN; }
void trigger(TR2, low) {     OT_TRIG2_PORT->ODR &= ~OT_TRIG2_PIN; }
void trigger(TR2, toggle) {  OT_TRIG2_PORT->ODR ^= OT_TRIG2_PIN; }
#else
