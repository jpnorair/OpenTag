/* Copyright 2009 JP Norair
  ...
  *
  */
/** @file       /OTplatform/~MCU_MSP430/MSP430F5/msp430f5_ucs.h
  * @author     JP Norair
  * @version    V1.0
  * @date       1 Dec 2009
...
#ifndef __MSP430F5_LIB_UCS_H
#define __MSP430F5_LIB_UCS_H
...
#define SELECT_FLLREF(source)   do { UCS->CTL3 = (UCS->CTL3 & ~(SELREF_7)) | (source); } while (0)

// Select source for ACLK    e.g. SELECT_ACLK(SELA__XT1CLK) */
#define SELECT_ACLK(source)   do { UCS->CTL4 = (UCS->CTL4 & ~(SELA_7)) | (source); } while (0)

// Select source for MCLK    e.g. SELECT_MCLK(SELM__XT2CLK) */
#define SELECT_MCLK(source)   do { UCS->CTL4 = (UCS->CTL4 & ~(SELM_7))   | (source); } while (0)

// Select source for SMCLK   e.g. SELECT_SMCLK(SELS__XT2CLK) */
#define SELECT_SMCLK(source)  do { UCS->CTL4 = (UCS->CTL4 & ~(SELS_7))   | (source); } while (0)

// Select source for MCLK and SMCLK e.g. SELECT_MCLK_SMCLK(SELM__DCOCLK + SELS__DCOCLK) */
#define SELECT_MCLK_SMCLK(sources) do { UCS->CTL4 = (UCS->CTL4 & ~(SELM_7 + SELS_7)) | (sources); } while (0)

// set ACLK/x */
#define ACLK_DIV(x)         do { UCS->CTL5 = (UCS->CTL5 & ~(DIVA_7)) | (DIVA__##x); } while (0)

// set MCLK/x */
#define MCLK_DIV(x)         do { UCS->CTL5 = (UCS->CTL5 & ~(DIVM_7)) | (DIVM__##x); } while (0)

// set SMCLK/x */
#define SMCLK_DIV(x)        do { UCS->CTL5 = (UCS->CTL5 & ~(DIVS_7)) | (DIVS__##x); } while (0)

// Select divider for FLLREF  e.g. SELECT_FLLREFDIV(2) */
#define SELECT_FLLREFDIV(x) do { UCS->CTL3 = (UCS->CTL3 & ~(FLLREFDIV_7))|(FLLREFDIV__##x); } while (0)
