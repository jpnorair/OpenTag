...
#if (OT_FEATURE(FSALLOC) && !OT_FEATURE(MALLOC))


#include <stdio.h>


///@todo tune-up this union for alignment.
...
static mem_header_t* get_mem_from_pool(uint16_t nquantas)
{
    uint16_t total_req_size;
...
//
void* memmgr_alloc(uint16_t nbytes)
{
    mem_header_t* p;
    mem_header_t* prevp;
    ...
    //
    uint16_t nquantas = (nbytes + sizeof(mem_header_t) - 1) / sizeof(mem_header_t) + 1;
