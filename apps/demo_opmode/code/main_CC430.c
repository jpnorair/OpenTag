/* Copyright 2011 JP Norair
  ...
  */
/**
  * @file       /apps/Demo_Opmode/Code/main_CC430.c
  * @author     JP Norair
  * @version    V1.0
  * @date       16 December 2011
  * @brief      Opmode Switching Demo for CC430
  *
  * This Application Requires:
  * <LI> A CC430 with 32KB Flash (or more) and 4KB SRAM (or more)       </LI>
...
#   define __UID    0x1D, 0xCC, 0x43, 0x0F, 0x51, 0x37, 0x00, 0x01
#   define __VID    0x1D, 0xCC
...
/** @note BETWEEN THIS NOTE AND THE SIMILAR ONE BELOW, THERE IS SOME PLATFORM-
  * DEPENDENT CODE.  Maybe at a later date I will clean it, like below.
  */
/*
#if (BOARD == AG430DK)
#   include "main_AG430DK.c"
#elif ...
#endif
*/
...
void sub_trig_init() {
    ...
    APP_TRIG3_PORT->DIR |= (APP_TRIG3_PIN | APP_TRIG4_PIN);
    APP_TRIG3_PORT->DS  |= (APP_TRIG3_PIN | APP_TRIG4_PIN);
#else
    APP_TRIG3_PORT->DIR |= APP_TRIG3_PIN;
    APP_TRIG3_PORT->DS  |= APP_TRIG3_PIN;
    APP_TRIG4_PORT->DIR |= APP_TRIG4_PIN;
...
void sub_button_init() {
  ...
  //APP_BUTTON_PORT->DIR   &= ~APP_BUTTON_PIN;
...
/** @note END OF PLATFORM-DEPENDENT CODE.
  */
...
void sub_led_cycle(ot_u8 i) {
    switch (i & 3) {
        case 0: sub_trig4_high();       break;  //non-stuffed
        case 1: sub_trig3_high();       break;  //red
        case 2: platform_trig2_high();  break;  //yellow
...
/// It is used here to turn-on activity LEDs.
    ...
    // Assume that (0 <= code1 <= 3), which is the case in normal operation
    if (code1 & 2)  platform_trig1_high();
    else            platform_trig2_high();
}
...
/// Hint: RFxRX_e1 label means the received frame has bad CRC.
    ...
    switch (code1) {
        ...
        /// (0) background scan ended, (1) or foreground scan ended
        case 0: loglabel[2] = 'B';
        case 1: loglabel[3] = 'R';
        ...
        /// Background Flood (2) or Foreground TX (3): turn-on green and log TX
        case 3: loglabel[2] = 'B';
        case 4: loglabel[3] = 'T';
                logdata_len = (app_devicemode == SYSMODE_GATEWAY) ? txq.length : 0;
                logdata     = txq.front;
                break;
...
const ot_u8 isf_stock_files[] = {
    ...
    0x10, 0x1F, 0x00, 0x40,                             /* NOTE: Scan Code should be less than     */
    0x10, 0x1F, 0x00, 0x80,                             /*       Next Scan, or else you will be    */
    0x10, 0x1F, 0x00, 0x40,                             /*       doing nothing except scanning!    */
    0x10, 0x1F, 0x00, 0x80,
    0x10, 0x1F, 0x00, 0xC0,
    0xFF, 0xFF, 0x00, 0xFF,
    0xFF, 0xFF, 0xFF, 0xFF,
    ...
    /* Period data format in Section X.9.4.5 of Mode 2 spec */
    0x10, 0x51, 0x00, 0x01,                             /* Channel X scan, Scan Code, Next Scan ms */
    0x10, 0x23, 0x00, 0xA0,
    0xFF, 0xFF, 0xFF, 0xFF,
    0xFF, 0xFF, 0xFF, 0xFF,
    0xFF, 0xFF, 0xFF, 0xFF,
