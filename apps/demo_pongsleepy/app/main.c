/* Copyright 2014 JP Norair
  ...
  */
/**
  * @file       /apps/demo_pongsleepy/code/main.c
  * @author     JP Norair
  * @version    R100
  * @date       27 Aug 2012
  * @brief      Demo of Pong with Low Power Listening (PongSleepy)
  *
...
void main(void) {
    ...
    ///   this app, and these setting could be hardcoded right in there

    ///4a. Set the app name (PongSleepy) as a cookie in the User-ID.
    ///    This is used for query filtering
    {   static const ot_u8 appstr[] = "APP=PongSleepy";
        vlFILE* fp;
        fp = ISF_open_su(ISF_ID(user_id));
        if (fp != NULL) vl_store(fp, sizeof(appstr), appstr);
        vl_close(fp);
    }
    
    ///4b. Set the sleep scan pattern. The data is a bit cryptic:
    ///    Byte0 = Channel ID (0x18)
    ///    Byte1 = Scan Code (0x80 indicates BG scan with minimal duration)
    ///    Byte2:3 = Scan interval (0x04,0x00 is 1024 = 1 second)
    {   static const ot_u8 sleep_pattern[] = { 0x18, 0x80, 0x04,0x00 };
        vlFILE* fp = ISF_open_su(ISF_ID(sleep_scan_sequence));
        if (fp != NULL) vl_store(fp, sizeof(sleep_pattern), sleep_pattern);
        vl_close(fp);
    }
