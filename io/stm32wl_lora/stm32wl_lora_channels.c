/* Copyright 2020 JP Norair
 * Licensed under the OpenTag License, Version 1.0 (the "License");
  * you may not use this file except in compliance with the License.
  * You may obtain a copy of the License at
  *
  * http://www.indigresso.com/wiki/doku.php?id=opentag:license_1_0
  *
  * Unless required by applicable law or agreed to in writing, software
  * distributed under the License is distributed on an "AS IS" BASIS,
  * WITHOUT WARRANTIES OR CONDITIONS OF ANY KIND, either express or implied.
  * See the License for the specific language governing permissions and
  * limitations under the License.
  */
/**
  * @file       /io/stm32wl_lora/stm32wl_lora_channels.c
  * @author     JP Norair
  * @version    R100
  * @date       31 Jan 2022
  * @brief      Channel Plans for different regulatory regimes
  * @ingroup    STM32WL_LoRa
  * 
  * @todo this file is a very incomplete port from SX127x to STM32WL LoRa.
  *       It is only used right now for compiling platform tests, and it
  *       will be overhauled shortly. 
  *
  ******************************************************************************
  */


#include <otstd.h>
#include <board.h>
#if defined(__STM32WL_LORA__)

#include <io/stm32wl_lora/interface.h>
#include <io/stm32wl_lora/config.h>

// For handling of phymac struct
#include <m2/radio.h>

/// LoRa channel changing requires changing the base frequency, as there is no
/// channel register for doing adjustments to the base frequency.  DASH7 over
/// LoRa uses the following channel rules:
///
/// 433 MHz band: 15x 116 kHz Subchannels from 433.05 - 434.79 MHz
/// 866 MHz band: 15x 250 kHz Subchannels from 865 - 869.250 MHz
/// 915 MHz band: 15x 1733 kHz Subchannels from 902 - 924.5 MHz

#define BASE_432M992Hz      0x1B0FDF3B
#define BASE_864M875Hz      0x360E0000
#define BASE_901M250Hz      0x38540000
#define BASE_901M133Hz      0x38522222
#define SPACE_116kHz        0x1DB22
#define SPACE_250kHz        0x40000
#define SPACE_1500kHz       0x180000
#define SPACE_1733kHz       0x1BBBBB
#define __FREQ32(FC)        (FC)
#define FREQ_SIZE           4


/// Bandplan tables must be 60 bytes each: 15 channels * 4 bytes each channel

/// CODE 0: US ISM band 915MHz
static const ot_u32 channels_ISM915[] = {
    __FREQ32(BASE_901M133Hz+(1*SPACE_1733kHz)),
    __FREQ32(BASE_901M133Hz+(2*SPACE_1733kHz)),
    __FREQ32(BASE_901M133Hz+(3*SPACE_1733kHz)),
    __FREQ32(BASE_901M133Hz+(4*SPACE_1733kHz)),
    __FREQ32(BASE_901M133Hz+(5*SPACE_1733kHz)),
    __FREQ32(BASE_901M133Hz+(6*SPACE_1733kHz)),
    __FREQ32(BASE_901M133Hz+(7*SPACE_1733kHz)),
    __FREQ32(BASE_901M133Hz+(8*SPACE_1733kHz)),
    __FREQ32(BASE_901M133Hz+(9*SPACE_1733kHz)),
    __FREQ32(BASE_901M133Hz+(10*SPACE_1733kHz)),
    __FREQ32(BASE_901M133Hz+(11*SPACE_1733kHz)),
    __FREQ32(BASE_901M133Hz+(12*SPACE_1733kHz)),
    __FREQ32(BASE_901M133Hz+(13*SPACE_1733kHz)),
    __FREQ32(BASE_901M133Hz+(14*SPACE_1733kHz)),
    __FREQ32(BASE_901M133Hz+(15*SPACE_1733kHz))
};

/// CODE 1: EU ISM band 866
static const ot_u32 channels_ISM866[] = {
    __FREQ32(BASE_864M875Hz+(1*SPACE_250kHz)),
    __FREQ32(BASE_864M875Hz+(2*SPACE_250kHz)),
    __FREQ32(BASE_864M875Hz+(3*SPACE_250kHz)),
    __FREQ32(BASE_864M875Hz+(4*SPACE_250kHz)),
    __FREQ32(BASE_864M875Hz+(5*SPACE_250kHz)),
    __FREQ32(BASE_864M875Hz+(6*SPACE_250kHz)),
    __FREQ32(BASE_864M875Hz+(7*SPACE_250kHz)),
    __FREQ32(BASE_864M875Hz+(8*SPACE_250kHz)),
    __FREQ32(BASE_864M875Hz+(9*SPACE_250kHz)),
    __FREQ32(BASE_864M875Hz+(10*SPACE_250kHz)),
    __FREQ32(BASE_864M875Hz+(11*SPACE_250kHz)),
    __FREQ32(BASE_864M875Hz+(12*SPACE_250kHz)),
    __FREQ32(BASE_864M875Hz+(13*SPACE_250kHz)),
    __FREQ32(BASE_864M875Hz+(14*SPACE_250kHz)),
    __FREQ32(BASE_864M875Hz+(15*SPACE_250kHz))
};

/// CODE 2: EU ISM 433
static const ot_u32 channels_ISM433[] = {
    __FREQ32(BASE_432M992Hz+(1*SPACE_116kHz)),
    __FREQ32(BASE_432M992Hz+(2*SPACE_116kHz)),
    __FREQ32(BASE_432M992Hz+(3*SPACE_116kHz)),
    __FREQ32(BASE_432M992Hz+(4*SPACE_116kHz)),
    __FREQ32(BASE_432M992Hz+(5*SPACE_116kHz)),
    __FREQ32(BASE_432M992Hz+(6*SPACE_116kHz)),
    __FREQ32(BASE_432M992Hz+(7*SPACE_116kHz)),
    __FREQ32(BASE_432M992Hz+(8*SPACE_116kHz)),
    __FREQ32(BASE_432M992Hz+(9*SPACE_116kHz)),
    __FREQ32(BASE_432M992Hz+(10*SPACE_116kHz)),
    __FREQ32(BASE_432M992Hz+(11*SPACE_116kHz)),
    __FREQ32(BASE_432M992Hz+(12*SPACE_116kHz)),
    __FREQ32(BASE_432M992Hz+(13*SPACE_116kHz)),
    __FREQ32(BASE_432M992Hz+(14*SPACE_116kHz)),
    __FREQ32(BASE_432M992Hz+(15*SPACE_116kHz))
};


/// Symbol Timing (miti) per spreading factor, per region
// ------------------------------------------
// SF11 @ 125 kHz = 16.384 ms/sym (17180)
// SF11 @ 250 kHz = 8.192 ms/sym (8590)
// SF11 @ 500 kHz = 4.096 ms/sym (4295)
// ------------------------------------------
// SF10 @ 125 kHz = 8.192 ms/sym (8590)
// SF10 @ 250 kHz = 4.096 ms/sym (4295)
// SF10 @ 500 kHz = 2.048 ms/sym (2148)
// ------------------------------------------
// SF9  @ 125 kHz = 4.096 ms/sym (4295)
// SF9  @ 250 kHz = 2.056 ms/sym (2148)
// SF9  @ 500 kHz = 1.024 ms/sym (1074)
// ------------------------------------------
// SF7  @ 41.7 kHz = 3.076 ms/sym (3222)
// SF7  @ 62.5 kHz = 2.048 ms/sym (2148)
// SF7  @ 125 kHz = 1.024 ms/sym (1074)
// SF7  @ 250 kHz = 0.512 ms/sym (537)
// SF7  @ 500 kHz = 0.256 ms/sym (269)
// ------------------------------------------
// SF5  @ 125 kHz = 0.256 ms/sym (269)
// SF5  @ 250 kHz = 0.128 ms/sym (135)
// SF5  @ 500 kHz = 0.064 ms/sym (68)
// ------------------------------------------
static const ot_u16 mitipersym_ISM915[] = {
    4295,
    1074,
    269,
    68
};
static const ot_u16 mitipersym_ISM866[] = {
    17180,
    4295,
    1074,
    269
};
static const ot_u16 mitipersym_ISM433[] = {
    8590,
    2148,
    537,
    135
};



static const ot_u16 tipermpc_ISM915[] = {
    51,     // 12 symbols, 11 bits each, 132 bits
    17,     // 16 symbols, 9 bits each, 144 bits
    6,      // 20 symbols, 7 bits each, 140 bits
    2       // 28 symbols, 5 bits each, 140 bits
};
static const ot_u16 tipermpc_ISM866[] = {
    202,
    68,
    21,
    8
};
static const ot_u16 tipermpc_ISM433[] = {
    101,
    34,
    11,
    4
};




/// Bandplan LUT
static const ot_u32* channel_map[] = {
    channels_ISM915,
    channels_ISM866,
    channels_ISM433,
    channels_ISM915,     // LUT must have a power-of-two regimes, default is ISM915
};


/// Bandwidth LUT
static const ot_u8 bandwidth_lut[] = {
    _BW_500_KHZ,
    _BW_125_KHZ,
    _BW_250_KHZ,
    _BW_500_KHZ,     // LUT must have a power-of-two regimes, default is ISM915
};


/// MiTi per symbol
static const ot_u16* mitipersym_lut[] = {
    (ot_u16*)mitipersym_ISM915,
    (ot_u16*)mitipersym_ISM866,
    (ot_u16*)mitipersym_ISM433,
    (ot_u16*)mitipersym_ISM915,    // LUT must have a power-of-two regimes, default is ISM915
};


/// Ti per smallest LoRa symbol group that contains 128 bits
static const ot_u16* tipermpc_lut[] = {
    (ot_u16*)tipermpc_ISM915,
    (ot_u16*)tipermpc_ISM866,
    (ot_u16*)tipermpc_ISM433,
    (ot_u16*)tipermpc_ISM915,    // LUT must have a power-of-two regimes, default is ISM915
};



const ot_u32* wllora_get_bandplan(ot_u8 region_code) {
    region_code &= 3;
    return channel_map[region_code];
}


const ot_u32 wllora_get_chanfreq(ot_u8 region_code, ot_u8 chan_ordinal) {
    const ot_u32* channel   = wllora_get_bandplan(region_code);
    const ot_u32 chanfreq   = channel[chan_ordinal];
    return chanfreq;
}


void wllora_configure_chan(ot_u8 region_code, ot_u8 chan_ordinal) {
    const ot_u32 chanfreq = wllora_get_chanfreq(region_code, chan_ordinal);

    // Write the frequency to the radio core
    wllora_rffreq_cmd(chanfreq);
}


ot_u8 wllora_get_bw(ot_u8 region_code) {
    region_code &= 3;

    return bandwidth_lut[region_code];
}




///@todo this is subject to change
ot_u16 wllora_symbol_miti(ot_u8 region_code, ot_u8 rate_code) {
    const ot_u16* mitipersym;
    region_code &= 3;
    rate_code   &= 3;

    mitipersym = mitipersym_lut[region_code];

    return mitipersym[rate_code];
}


///@todo this is subject to change
ot_u16 wllora_block_miti(const void* phy_handle) {
    const phymac_struct* pm = phy_handle;
    ot_u8 regime;
    ot_u8 rcode;
    ot_u16 miti;

    regime  = pm->flags & 3;
    rcode   = (pm->channel >> 4) & 3;
    miti    = (mitipersym_lut[regime])[rcode];
    miti   *= (pm->channel & 0x80) ? 4 : 5;

    return miti;
}


ot_u16 wllora_mpcblock_ti(const void* phy_handle) {
    const phymac_struct* pm = phy_handle;
    ot_u8 regime;
    ot_u8 rcode;

    regime  = pm->flags & 3;
    rcode   = (pm->channel >> 4) & 3;
    return (tipermpc_lut[regime])[rcode];
}


#endif


