...
//
#ifdef RFCORE_DEBUG
#   define __CORE_DUMP()   sx127x_coredump(rfcore_dump, 112)
...
#       endif
    ...
    //sx127x_write(RFREG_LR_SYMBTIMEOUTLSB, initvals[2]);
...
OT_WEAK void rm2_rxtest(ot_u8 channel, ot_u8 tsettings, ot_u16 timeout) {
    ...
    sx127xdrv_mdmconfig(fginit[0], fginit[1], fginit[2], 0);
...
/// Continues where rm2_txcsma() leaves off.
    ...
    if ((rfctl.flags & RADIO_FLAG_BGFLOOD) == RADIO_FLAG_BGFLOOD) {
        ...
        if ((rfctl.state & RADIO_STATE_TXMASK) == RADIO_STATE_TXDATA) {
            ...
            //sx127x_antsw_tx((ot_bool)(BOARD_FEATURE(RF_PABOOST)));             ///@todo make sure this doesn't need to be here
            sx127x_strobe(_OPMODE_TX, False);   //non-blocking call
            return;
        }
    }
...
#ifndef EXTF_sx127xdrv_save_linkinfo
OT_WEAK void sx127xdrv_save_linkinfo(void) {
//    // Link information: only SNR is available, which is saved as LQI
//    radio.link.pqi  = 1;
//    radio.link.sqi  = 1;
//    radio.link.lqi  = sx127x_read(RFREG_LR_PKTSNRVALUE);
//    radio.link.agc  = 0;
}
#endif
