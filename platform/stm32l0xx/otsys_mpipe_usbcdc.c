...
#define __USB_CLKON()   BOARD_USBCLK_ON()
#define __USB_CLKOFF()  BOARD_USBCLK_OFF()
...
#define USB_Cable_Config_ENABLE  (USB->BCDR |= (ot_u32)USB_BCDR_DPPU)
#define USB_Cable_Config_DISABLE (USB->BCDR &= ~(ot_u32)USB_BCDR_DPPU)
...
void platform_isr_usb(void) {
    ...
    ot_u32 i    = 0;
    wIstr       = _GetISTR();
    istr_masked = wInterrupt_Mask & wIstr;
    
#   if (IMR_MSK & ISTR_SOF)
        if (istr_masked & ISTR_SOF) {
...
/** USB Wakeup interrupt   <BR>
  ...
  */
///@todo not sure where this goes in STM32L0
