...
void HAL_PCD_MspInit(PCD_HandleTypeDef *hpcd) {
    ...
    {   uint32_t scratch;
        scratch         = GPIOA->MODER;
        scratch        &= ~((3 << (11*2)) | (3 << (12*2)));
        scratch        |= ((2 << (11*2)) | (2 << (12*2)));
        GPIOA->MODER    = scratch;
        
        scratch         = GPIOA->AFR[1];
        scratch        &= ~((15 << (3*4)) | (15 << (4*4)));
        scratch        |= ((GPIO_AF0_USB << (3*4)) | (GPIO_AF0_USB << (4*4)));
        
        GPIOA->PUPDR   &= ~((3 << (11*2)) | (3 << (12*2)));
...
/** @todo update this function to work with OpenTag's sleeping
  * @brief  SOF callback.
  * @param  hpcd: PCD handle
  * @retval None
  */
...
uint8_t USBD_LL_IsStallEP(USBD_HandleTypeDef *pdev, uint8_t ep_addr) {
    ...
    ep  = (ep_addr & 0x80) ? hpcd->IN_ep : hpcd->OUT_ep;
    
    return ep[ep_addr & 0x7f].is_stall;
}
...
void USBD_LL_Delay(uint32_t Delay) {
    //HAL_Delay(Delay);
    delay(Delay);       ///@note uses otlib delay function
}
