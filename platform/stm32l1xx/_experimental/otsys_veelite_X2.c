/* Copyright 2010-2013 JP Norair
  ...
  */
/**
  * @file       /otplatform/stm32l1xx/veelite_core_X2_STM32L1xx.c
  * @author     JP Norair
  * @version    R100
  * @date       17 Jan 2013
...
/** MACROS for STM32L1xx library <BR>
  * ========================================================================<BR>
  * @note If you are upgrading the STM32 FW lib, you need to modify the file 
  * stm32l1xx_flash.h file in a simple way.  The FLASH_Status enum type needs to
...
ot_u16 vworm_read(vaddr addr) {
    ...
    SEGFAULT_CHECK(addr, in_vworm, 7, "VLC_461");   //__LINE__
...
ot_u8 vworm_write(vaddr addr, ot_u16 data) {
    ...
    SEGFAULT_CHECK(addr, in_vworm, 7, "VLC_489");   //__LINE__
...
ot_u8 vworm_mark_physical(ot_u16* addr, ot_u16 value) {
    ...
    BUSERROR_CHECK( (((ot_u16)addr < VWORM_BASE_PHYSICAL) || \
                    ((ot_u16)addr >= (VWORM_BASE_PHYSICAL+VWORM_ALLOC))), 7, "VLC_576");    //__LINE__
...
ot_u16 vsram_read(vaddr addr) {
    ...
    return 0;
#else
    SEGFAULT_CHECK(addr, in_vsram, 7, "VLC_600");   //__LINE__
...
ot_u8 vsram_mark(vaddr addr, ot_u16 value) {
    ...
    return ~0;
#else
    SEGFAULT_CHECK(addr, in_vsram, 7, "VLC_615");   //__LINE__
    addr 		   -= VSRAM_BASE_VADDR;
    vsram[addr>>1]  = value;
    return 0;
#endif
...
#else
    ot_u8* output;
    SEGFAULT_CHECK(addr, in_vsram, 7, "VLC_643");   //__LINE__
    addr   -= VSRAM_BASE_VADDR;
