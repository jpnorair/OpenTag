/* Copyright 2013 JP Norair
  ...
  */
/**
  * @file       /board/stm32l1xx/board_HayTag_USB.h
  * @author     JP Norair
  * @version    R100
  * @date       29 Aug 2013
  * @brief      Board Configuration for HayTag USB Stick
  * @ingroup    Platform
  *
...
#ifndef __board_HayTag_USB_H
#define __board_HayTag_USB_H
...
// I2C interface, not presently utilized
...
/// B. Random Number ADC pins: A Zener can be used to generate noise.
    ...
    // - A9 is UART TX, which is ALT push-pull output, and which needs pull-up
    ...
    */
    ...
    GPIOA->PUPDR    = (2 << (BOARD_RFSPI_MISOPINNUM*2)) \
                    | /*(1 << (BOARD_UART_TXPINNUM*2)) | */(1 << (BOARD_UART_RXPINNUM*2)) \
                    | (1 << (13*2)) | (2 << (14*2));
    ...
    */
    ...
    // - B4:5 are TEST pins, which are set to output ground
    ...
    //                | (GPIO_MODER_OUT << (8*2)) \
    //                | (GPIO_MODER_OUT << (10*2));
    GPIOB->MODER    = (GPIO_MODER_OUT << (4*2)) \
                    | (GPIO_MODER_OUT << (5*2)) \
                    | (GPIO_MODER_OUT << (8*2)) \
                    | (GPIO_MODER_OUT << (10*2));
...
#   define __USE_EXTI9
        ...
        (BOARD_FEATURE_MPIPE_BREAK && (BOARD_UART_RXPINNUM == 10)) || \
