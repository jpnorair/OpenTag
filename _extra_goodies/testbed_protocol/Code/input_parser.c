...
int sub_getdecblock(FILE* stream, ot_queue* msg);
int sub_gethexnum(int* status, FILE* stream, ot_queue* msg);
...
int parse_sequence(FILE* stream, ot_queue* msg) {
    int status;
...
int sub_getbinnum(int* status, FILE* stream, ot_queue* msg) {
    int     digits;
    int     i = 0;
    int     shift;
    char    next;
